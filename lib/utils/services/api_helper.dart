...
import 'package:AstrowayCustomer/model/home_Model.dart' as home_model;
...
class APIHelper {
  ...
  Future<dynamic> loginSignUp(LoginModel loginModel) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        global.showToast(
            message: '${recordList['error']['contactNo']}',
            textColor: textColor,
  ...
  Future<dynamic> checkContact(String phoneno) async {
    try {
      final response = await http.post(
        ...
        headers: await global.getApiHeaders(false),
        body: json.encode({"contactNo": phoneno}),
      );
      dynamic recordList;
  ...
  Future<dynamic> getHomeBanner() async {
    try {
      ...
      dynamic recordList;
      if (response.statusCode == 200) {
        recordList = List<home_model.Banner>.from(json
            .decode(response.body)["banner"]
            .map((x) => home_model.Banner.fromJson(x)));
      } else {
        recordList = null;
      }
      return getAPIResult(response, recordList);
