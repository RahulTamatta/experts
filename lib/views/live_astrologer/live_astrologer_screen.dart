 ...
 //MOBILE SUPPORT
  void sendChannelMessage(String messageText, String? gift) async {
    try {
      ...
      var (status, response) = await client.publish(
        _channelname,
        '${currentUserName}&&$messageText&&$currentUserProfile&&$isme&&$gift',
  ...
  void createClient() async {
    try {
      ...
      log(
        'chat token is ${global.agoraChatToken} and channel name is $_channelname',
      );
  ...
  void _setupListeners() {
    ...
    client.addListener(
      message: (event) {
        ...
        log(
          '_setupListeners msg ${_messageparts[1]} and ${_messageparts[0]} and ${_messageparts[2]}',
        );
      ...
      linkState: (event) {
        log(
          '[Link State Changed] From: ${event.previousState} → ${event.currentState}',
        );
